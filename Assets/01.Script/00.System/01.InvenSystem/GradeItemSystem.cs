...
public class GradeItemSystem //아이템 합치기 줄이기
{
    public const int ItemCount = 32;
    
    public GradeItemEntry[] Entries = new GradeItemEntry[ItemCount]; //아이템 칸  배열 설정
    public GradeItemEntry[] GetSlots() { return Entries; }
    public UserDataController m_Data;

    
    public void LoadToInventory(int _arrayNum, string _iTemName, int _itemNum, GameData Owner) 
    {
      for (int i = 0; i < Owner.gameItemDataBase.GetGradetItemLenth(); i++)
        if(Owner.gameItemDataBase.GetGradeItem(i).ItemName == _iTemName)
        {
            Entries[_arrayNum] = new GradeItemEntry();
            Entries[_arrayNum].Item = Owner.gameItemDataBase.GetGradeItem(i);
            Entries[_arrayNum].Count = _itemNum;
        }
    }
    public void AddGradeItem(GradeItem item) //아이템 추가

    {
        ...
        {
            GradeItemEntry entry = new GradeItemEntry(); //아이템 변수 생성하고
    ...
    public bool GradeItemChecking(List<GradeItemEntry> ItemCheckArray)
    {
        if (Entries == null)
        {
    ...
    public void MinusGradeItem(List<GradeItemEntry> ItemCheckArray)
    {
        for (int i = 0; i < ItemCheckArray.Count; i++) //필요 아이템 전체 순회
        {
            ...
            {
                if (Entries[j] == null)
                { Debug.Log("MinusItem  PlayerInventory 일부에서 null포인터 감지 GradeItemSystem.csharp"); return; }
                else if (ItemCheckArray[i].Item.ItemName == Entries[j].Item.ItemName)
                { // 인벤토리[j]번째 아이템이 필요 아이템과 같다면 
                    for (int k = 0; k < ItemCheckArray[i].Count; k++)
                    {   //[i]번째 아이템의 소모량만큼 반복
                        MinusGradeItem(j);//인벤토리 j번째 아이템 감소
                    }
                    break;
                }
            }
    ...
    public bool MinusGradeItem(int GradeItemEntry) //아이템을 1개 감소시킨 뒤 개수가 0이하면 데이터를 삭제.
    {
        if (Entries[GradeItemEntry] != null)
        {
            Entries[GradeItemEntry].Count -= 1;

            if (Entries[GradeItemEntry].Count <= 0) //차감한 아이템 개수가 0이하면
            {
                for (int i = 0; i < 32; ++i) //아이템 칸수만큼 반복적으로 수행
                    if (Entries[i] == Entries[GradeItemEntry]) //사용한 아이템이 있는 칸이면
                    {
    ...
    public void ReSort()
    {
         GradeItemEntry[] Save = new GradeItemEntry[32];
         int Num = 0;
         for (int i = 0; i < 32; ++i)
         {
             if (Entries[i] != null)
