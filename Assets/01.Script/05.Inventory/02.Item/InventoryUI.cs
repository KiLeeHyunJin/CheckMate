...
using UnityEngine.UI;
    ...
    /// </summary>
public class InventoryUI : MonoBehaviour  //인벤토리 칸 UI
{
    public class DragData //드레그에 사용할 아이템의 정보를 저장할 클래스
    {
        public ItemEntryUI DraggedEntry; //아이템 드래그, 이미지, 개수, 더블클릭, 장착 베이스 함수 
        public RectTransform OriginalParent; //UI위치
    }
    
    public RectTransform[] ItemSlots; //아이템 슬롯 - UI이미지 전부 할당
    
    public ItemEntryUI ItemEntryPrefab; //아이템 칸 - 프리펩 하나만 할당
    ...
    //[Header("")]
    public ItemEntryUI m_HoveredItem;
    ...
    ItemEntryUI[] m_ItemEntries;//아이템 슬롯 데이터 복사
    public InventorySystem m_Data { get; set; }

    public void Start()
    {
        m_Data = FindObjectOfType<UserDataController>().invenSystem;
        DragCanvasScaler = DragCanvas.GetComponentInParent<CanvasScaler>(); // 드레그에 사용할 캔버스 오브젝트 할당
        m_ItemEntries = new ItemEntryUI[ItemSlots.Length]; // 아이템 슬롯 크기 할당
    ...
    private void TestMinusItem()
    {
        ...
        InventoryEntry sour = new InventoryEntry();
        sour.Count = 1;
        sour.Item = m_Data.m_Data.gameData.gameItemDataBase.GetItem(0);
        temp.Add(sour);

        InventoryEntry Dest = new InventoryEntry();
        Dest.Count = 1;
        Dest.Item = m_Data.m_Data.gameData.gameItemDataBase.GetItem(1);
        temp.Add(Dest);
        m_Data.MinusItem(temp);
    }

    private void TestAddItem()
    {
        Item NumOne  = m_Data.m_Data.gameData.gameItemDataBase.GetItem(0);
        if (NumOne != null)
            m_Data.AddItem(NumOne);
        else
        {
            Debug.Log("아이템 데이터가 없습니다. Index Num : 0");
            return;
        }
        NumOne = m_Data.m_Data.gameData.gameItemDataBase.GetItem(1);
        if (NumOne != null)
            m_Data.AddItem(NumOne);
        else
            return;
    ...
    public InventorySystem Owner // 캐릭터 데이터 프로퍼티
    {
        get { return m_Data; }
    }
    void OnEnable()
    ...
    public void Load(InventorySystem data) // 캐릭터 데이터 가져오기
    {
        //m_Data = data; // 매개변수 복사
        for (int j = 0; j <= Owner.Entries.Length; j++)
        {
            if (Owner.Entries[j] != null)
                m_ItemEntries[j].gameObject.SetActive(true);
        }
        ...
        for (int i = 0; i < ItemSlots.Length; ++i) //아이템 슬롯 길이만큼 반복
        {
            ...
            if (RectTransformUtility.RectangleContainsScreenPoint(t, position)) //마우스 포인터가 특정 범위안에 있는지 확인
            {
                if (m_ItemEntries[i] != CurrentlyDragged.DraggedEntry) //현재 드레그하고 있는 아이템이 해당 슬롯의 아이템과 같지 않다면
                {
                    ...
                    //아이템 스왑 코드
                    var prevItem = m_Data.Entries[CurrentlyDragged.DraggedEntry.InventoryEntry]; 
                    m_Data.Entries[CurrentlyDragged.DraggedEntry.InventoryEntry] = m_Data.Entries[i];
